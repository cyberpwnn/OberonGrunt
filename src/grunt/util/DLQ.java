...
public class DLQ
{
	...
	public void update(DL d)
	{
		...
		Client.ps.updateProgress(progress, "Downloading", c, k, totalSize - downloadedSize);

		if(d.state.equals(DLState.FAILED))
		{
			System.out.println("Download FAILED -> " + d.url.toString() + " > " + d.dest.getAbsolutePath());
			running.remove(d);
		}

		if(d.state.equals(DLState.FINISHED))
		{
			System.out.println("Download FINISHED -> " + d.dest.getAbsolutePath());
			c++;
			running.remove(d);
		}
	}
